...
def worker_routine(proccess_id_string, worker):
    ...
    while True:
        ...
        training_process_ps_py = subprocess.check_output('ps -elf | grep "{}"'.format("py"), encoding='utf-8', shell=True)
        print("training_process_ps_py: {}".format(training_process_ps_py))
        ...
        if not training_process_started:
            if training_process_running:
                ...
                print('Worker {} exiting: training not started in 180 seconds.'.format(worker))
